...
def parse_detail_url(url):
    ...
    # 类型
    types = re.findall('<span property="v:genre">(.*?)</span>', res)
    ...
    releaseTime = re.findall('<span class="pl">上映日期:</span>.*?content="(.*?)">.+?</span>', res)
...
def save_data(data):
    ...
    # 打印下载的个数
    nums = [num + 1 for num in range(25)]
    for num in nums:
